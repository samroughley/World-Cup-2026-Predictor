...
def run_single_knockout_iteration(args):
    """
    Performs one knockout simulation
    """
    np.random.seed()

    group_stage_outcomes, inverse_groups, third_place_matching, knockout_matches = args 


    # Determine the results from the group stage
    selected_group_stage_outcomes = {}
    for group_name, outcome_dist in group_stage_outcomes.items():
    ...
    one_run_simulation_results = {t: {"R32": 0, "R16": 0, "QF": 0, "SF": 0, "F": 0, "Winner": 0} for t in inverse_groups.keys()}
    one_run_simulation_results = simulate_knockout_rounds(knockout_spots, knockout_matches, one_run_simulation_results)

    return one_run_simulation_results


def parallel_sim(num_iterations, checkpoint_freq):
    
    # Prepare data for workers
    worker_args = (group_stage_results, inverse_groups, third_place_matching, knockout_matches)

    master_simulation_results = {t: {"R32": 0, "R16": 0, "QF": 0, "SF": 0, "F": 0, "Winner": 0} for t in inverse_groups.keys()}

    with Pool(processes=2) as pool:
        # imap_unordered is great for tqdm and speed
        for i, one_run_outcome in enumerate(tqdm(pool.imap_unordered(run_single_knockout_iteration, [worker_args]*num_iterations), total=num_iterations)):
            
            # --- THE UPDATE STEP ---
            # Now the main process updates the master object one-by-one
            # This avoids all race conditions and sync issues
            # for stage, winner in one_run_outcome.items():
            #     master_simulation_results[stage][winner] += 1
            for team, outcomes in one_run_outcome.items():
                for round_reached, count in outcomes.items():
                    master_simulation_results[team][round_reached] += count
            ...
            if (i + 1) % checkpoint_freq == 0:
                ...
                results_for_saving = {}
                for k, v in master_simulation_results.items():
